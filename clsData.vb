...
Imports Transact.Data.Core.Common
...
ByVal SSN As String, ByVal Sex As String, ByVal TransactionType As String, ByVal BK_Flag As Integer, ByVal PAN As String) As Integer
    ...
    Public Function GetCustomerDataFromID(ByVal strPhotoID As String, ByVal dDOB As DateTime, ByVal strState As String,
        ...
        While dataReader.Read
            ...
            End If
            root.AppendChild(xmlEle13)
            Dim xmlEle14 As XmlNode = xmlDoc.CreateNode(XmlNodeType.Element, "Fee1", "")
            If Not IsDBNull(dataReader("Fee1")) Then
                xmlEle14.InnerText = dataReader("Fee1")
            Else
                xmlEle14.InnerText = 0
            End If
            root.AppendChild(xmlEle14)
            Dim xmlEle15 As XmlNode = xmlDoc.CreateNode(XmlNodeType.Element, "PAN", "")
            If Not IsDBNull(dataReader("PAN")) Then
                xmlEle15.InnerText = dataReader("PAN")
            Else
            ...
            End If
            root.AppendChild(xmlEle15)
            Dim xmlEle16 As XmlNode = xmlDoc.CreateNode(XmlNodeType.Element, "Fee2", "")
            If Not IsDBNull(dataReader("Fee2")) Then
                xmlEle16.InnerText = dataReader("Fee2")
            Else
                xmlEle16.InnerText = 0
            End If
            root.AppendChild(xmlEle16)
            Dim xmlEle17 As XmlNode = xmlDoc.CreateNode(XmlNodeType.Element, "Fee3", "")
            If Not IsDBNull(dataReader("Fee3")) Then
                xmlEle17.InnerText = dataReader("Fee3")
            Else
                xmlEle17.InnerText = 0
            End If
            root.AppendChild(xmlEle17)
            Dim xmlEle18 As XmlNode = xmlDoc.CreateNode(XmlNodeType.Element, "CheckDate", "")
            If Not IsDBNull(dataReader("CheckDate")) Then
                xmlEle18.InnerText = dataReader("CheckDate")
            Else
    ...
    Public Function GetAllTransactionsbyBlockAndCust(ByVal iBlockID As Integer, ByVal iCustID As Integer, ByVal iTranID As Integer, ByVal acctDs As DataSet, ByVal feeCompDS As DataSet) As clsBlockDetails
        ...
        For Each oIFXRow In dsIFX.Tables(0).Rows
            ...
            For Each oIFXCol In dsIFX.Tables(0).Columns
                ...
                ElseIf oIFXCol.Caption = "pos_auth_cycle" Then
    ...
    Public Function GetAllTransactionsbyAccount(ByVal iActID As Integer) As List(Of clsTranRow)
        Dim dt As New DataTable("AllTransactions")
        Dim arrIFXAcct As New ArrayList()
        ...
        Dim arrTrans As New ArrayList
        Dim dsIFX As DataSet = IFX_GetAllTransByActNbr(actNbr)
        For Each oIFXRow In dsIFX.Tables(0).Rows
            Dim oAcct As New clsAccount()
            For Each objRow In acctDS.Tables(0).Rows
                For Each objCol In acctDS.Tables(0).Columns
            ...
            For Each oIFXCol In dsIFX.Tables(0).Columns
                ...
                ElseIf oIFXCol.Caption = "acct_2_nbr" Then
                    oAcct.oTran.Act2Nbr = oIFXRow(oIFXCol)
                    If oAcct.oTran.ActNbr = actNbr.PadLeft(18, "0") Then
                        bAcctmatch = True
                        oAcct.oTran.ActNbr = actNbr.PadLeft(18, "0")
                    End If
                    If oAcct.oTran.Act2Nbr = actNbr.PadLeft(18, "0") Then
                        bAcct2match = True
                    End If
                    If bAcctmatch And Not bAcct2match Then
                        If oAcct.CustomerID <> oAct1Acct.CustomerID Then
    ...
    End Function
    Public Function IFX_RetrieveBarcodeForPan(ByVal Bin As String, ByVal PanLastFour As String) As Integer
        Dim barcode As String = ""
        oConn.Connect(True)
        Dim sqlCmd As New IfxCommand("select barcode from cardholder where (cardholder = " + PanLastFour + " And Bin = " + Bin + ")", oConn.ifxConn)
        sqlCmd.CommandType = CommandType.Text
        Dim dataReader = sqlCmd.ExecuteReader()
    ...
    End Function
    Public Function IFX_RetrieveCompanyNumberForPan(ByVal Bin As String, ByVal PanLastFour As String) As Integer
        Dim branch_nbr As Integer
        oConn.Connect(True)
        Dim sqlCmd As New IfxCommand("select branch_nbr from cardholder where (cardholder = " + PanLastFour + " And Bin = " + Bin + ")", oConn.ifxConn)
        sqlCmd.CommandType = CommandType.Text
        Dim dataReader = sqlCmd.ExecuteReader()
        While dataReader.Read
    ...
    Public Function IFX_GetSrcTransactionsByAcct(ByVal iActID As Integer, ByVal strActNbr As String) As DataSet
        ...
        Dim sqlCmd As New IfxCommand("SELECT tp_reg_e as description,type_code, pos_auth_cycle, acct_1_nbr,acct_2_nbr,tp_seq as tranID, bin, cardholder, amount_auth, tp_datetime, pos_batch_nbr, pri_tran_code, sec_tran_code " + _
    ...
    Public Function IFX_GetTransactionsByBlock(ByVal iBlockID As Integer, ByVal iCustID As Integer) As DataSet
        ...
        oConn.Connect(True)
        Dim sqlCmd As New IfxCommand("SELECT amount_req,pos_auth_cycle,bal_1 as bal1,tp_seq as tranid,amount_auth,acct_1_nbr,acct_2_nbr,tp_datetime,pos_auth_cycle, tp_reg_e as description,type_code, chp_fee,bin,  cardholder,   pos_batch_nbr, pri_tran_code, sec_tran_code, " + _
        ...
        "WHERE (t2.primarytrancode = trim(t1.pri_tran_code) and t2.secondarytrancode = trim(t1.sec_tran_code) and t3.tran_status = t1.type_code) and (t1.pos_auth_cycle = " + iBlockID.ToString() + " and t1.pos_batch_nbr = " + iCustID.ToString() + ")", oConn.ifxConn)
        sqlCmd.CommandType = CommandType.Text
        Dim da As New IfxDataAdapter
        da.SelectCommand = sqlCmd
        da.Fill(ds)
    ...
    Public Function IFX_GetTransactionsByTranID(ByVal iTranID As Integer) As DataSet
        Dim ds As New DataSet
        oConn.Connect(True)
        Dim sqlCmd As New IfxCommand("SELECT tp_reg_e as description,type_code,  bal_1,bal_2,chp_fee,acct_1_nbr,acct_2_nbr,tp_seq as tranID, bin, cardholder, amount_auth, tp_datetime, pos_batch_nbr, pri_tran_code, sec_tran_code, " + _
 "t2.acct1icon, t2.acct2icon, t2.custisdest, t2.description as TransactionTypeDescription, t2.description2 as TransactionTypeDescription2, t3.icon, t3.short_desc " + _
                          "FROM swx.log_record t1, swx.tgs_transactiontypes t2, swx.tgs_tran_status_desc t3  " + _
    ...
    Public Function IFX_GetAllTransByCustID(ByVal iCustID As Integer, ByVal ActNbrDS As ArrayList, ByVal CardNbrDS As DataSet, _
        ...
        Next
        'distinct case when count(pos_auth_cycle) > 1 then pos_auth_cycle else 0 end as pos_auth_cycle
        'select case when count(pos_auth_cycle) < 2 then 0 else pos_auth_cycle end as pos_auth_cycle
        oConn.Connect(True)
        Dim strSQL As String = "SELECT  amount_req,amount_auth,pos_batch_nbr,acct_1_nbr,acct_2_nbr,chp_fee,tp_datetime,pos_auth_cycle, tp_reg_e as description,type_code, chp_fee,bin,  cardholder,   pri_tran_code, sec_tran_code, " + _
    ...
    Public Function IFX_GetAllTransByActNbr(ByVal actNbr As String) As DataSet
        ...
        'select case when count(pos_auth_cycle) < 2 then 0 else pos_auth_cycle end as pos_auth_cycle
        oConn.Connect(True)
        Dim strSQL As String = "SELECT    pos_batch_nbr,chp_fee,amount_auth,acct_1_nbr,acct_2_nbr,tp_datetime,pos_auth_cycle, tp_reg_e as description,type_code, chp_fee,bin,  cardholder, pri_tran_code, sec_tran_code, " + _
                               "t2.acct1icon, t2.acct2icon, t2.custisdest, t2.description as TransactionTypeDescription, t2.description2 as TransactionTypeDescription2, t3.icon, t3.short_desc " + _
                               "FROM swx.log_record t1, swx.tgs_transactiontypes t2, swx.tgs_tran_status_desc t3  " + _
